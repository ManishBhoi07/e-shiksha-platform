...
const Infor = () => {
  ...
  // Initialize SpeechSynthesisUtterance single instance
  useEffect(() => {
    const utterance = new SpeechSynthesisUtterance(text);
    // When audio finishes naturally, update state
  ...
  const speakFeature = (featureText) => {
    // Stop main audio if they hover over a feature
    window.speechSynthesis.cancel();
    setIsPlaying(false);
    const featureUtterance = new SpeechSynthesisUtterance(featureText);
    window.speechSynthesis.speak(featureUtterance);
  }
        ...
        <div className="mx-auto grid max-w-2xl grid-cols-1 items-center gap-y-16 gap-x-8 py-24 px-4 sm:px-6 sm:py-32 lg:max-w-7xl lg:grid-cols-2 lg:px-8">
          <div>
            <div className="flex items-center gap-4">
              <h2 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl" onMouseOver={() => speakFeature("Why Shiksha?")}>Why Shiksha?</h2>
              <button onClick={toggleAudio} className="focus:outline-none hover:opacity-80 transition-opacity">
                <img src={isPlaying ? mute : vol} alt="toggle volume" className="w-8 h-8" />
              </button>
            </div>
            ...
            <dl className="mt-16 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 sm:gap-y-16 lg:gap-x-8">
              {features.map((feature) => (
                <div key={feature.name} className="border-t border-gray-200 pt-4 cursor-pointer" >
                  <dt className="font-medium text-gray-900" onMouseOver={() => speakFeature(feature.name)}>{feature.name}</dt>
                  <dd className="mt-2 text-sm text-gray-500" onMouseOver={() => speakFeature(feature.description)}>{feature.description}</dd>
                </div>
              ))}
            </dl>
          </div>
